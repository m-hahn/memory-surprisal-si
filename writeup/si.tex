...
Also, we excluded three morphemes that are treated as independent words, not suffixes, by \cite{kaiser2013japanese} (\textit{dekiru} `be able to', \textit{naru} `become', \textit{yoo} `as if').
Also, passive and potential markers are formally identical for many verbs; we included both here.
Also, we took \textit{suru} to be a unit with the preceding element; it is suffixed obligatorily to Sino-Japanese words when they are used as verbs.
...
Note that there  is no universally accepted segmentation for Japanese suffixes; we follow the UD tokenization in choosing which suffixes to segment.\footnote{The biggest difference to some other treatments is that the ending -u/-ru is viewed as part of the preceding morpheme that appears in some environments due to allomorphic variation, while it is viewed as a suffix in some other treatments.}



\begin{enumerate}
\item VALENCE: causative (-ase-). \cite[142]{hasegawa2014japanese} \cite[Chapter 13]{kaiser2013japanese}. In the UD data, this is lemmatized as \textit{saseru}, \textit{seru} (190 occurrences).
\item VOICE: passive (-are-, -rare-) \cite[152]{hasegawa2014japanese} \cite[Chapter 12]{kaiser2013japanese}  In the UD data, this is lemmatized as \textit{rareru}, \textit{reru} ($\approx$ 2000 occurrences).
\item MOOD, MODALITY:
\begin{enumerate}
\item potential (allomorphs -are-, -rare-, -e-). In the UD data, this is lemmatized as \textit{rareru}, \textit{reru}, \textit{eru}, \textit{keru}.
...
\item politeness -mas- (allomorphs -masu-, -mashi-, -mase-) \cite[190]{kaiser2013japanese}. % 見できます, 見ましたい
In the UD data, this is lemmatized as \textit{masu} ($\approx$ 600 occurrences).


\item MODALITY: desiderative -ta- (allomorphs: -tai, -taku-, -taka-) (85 occurrences) \cite[238]{kaiser2013japanese}. %mi-mashi-tai `I want to see'
\end{enumerate}

\item NEGATION: negation -na- (allomorphs: -nai, -n-, -nakaC-). 
...
\begin{enumerate}
\item ta for past ($\approx$ 4000 occurrences) 

\item yoo for hortative, future, and similar meanings \cite[229]{kaiser2013japanese}. This is lemmatized as \textit{u} (92 occurrences).
\end{enumerate}
...
The Demuth corpus has very extensive manual morphological segmentation and annotation; each verb form is segmented into morphemes, which are annotated for their function.
...
We obtained 37K verb forms.

Here, we describe how we extracted frequent morphemes and arrived at the description provided in the main paper.
...
We classified morphemes by this annotation. We considered morpheme types occurring at least 50 times in the corpus.
...
\begin{enumerate}
    ...
    This morpheme encodes agreement with the subject, for person, number, and noun class (the latter only in the 3rd person) \cite[\textsection 395]{doke1967textbook}.
    ...
    In various TAM forms, negation is encoded with a morpheme in this position (362 occurrences).
    Common allomorphs in the corpus include ska, seka, sa, skaba.
    ...
    Tense/aspect marker (t\^{} 13K) \cite[p. 165]{guma1971handbook} \cite[p. 109]{lombard1969handbook}
    
    Common TAM markers in this position in the corpus include:
    
    \begin{itemize}
    \item -tla-, -tlo-, -ilo- FUT \cite[\textsection 410]{doke1967textbook}
    \item -a- PRS \cite[\textsection 400]{doke1967textbook}
    \item -ka- POT \cite[\textsection 424]{doke1967textbook}
    \item -sa- PERSIST \cite[\textsection 414]{doke1967textbook}
    \item -tswa- RECENT.PST \cite[\textsection 405]{doke1967textbook} 
    \item 
    ..
    Past subjunctive
    ...
    \item OBJECT agreement (om 6K) or reflexive (rf, 751).
    
    Similar to subject agreement, object agreement denotes person, number, and noun class features of the object.
    Unlike subject agreement, it is optional (CITE).
    
    Object agreement and reflexive marking are mutually exclusive \cite[p. 165]{guma1971handbook}.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item causative (labeled \textit{c}, 1K occurrences), -isa (with morphophonological change) \cite[\textsection 325]{doke1967textbook}
    
    See \cite[\textsection 345]{doke1967textbook} for reversive+causative.
    
    \item neuter/stative (labeled \textit{nt}, 229 occurrences), -eha, -ahala \cite[\textsection 307]{doke1967textbook}
    
    
    \item reversive (labeled \textit{rv}, 214 occurrences),  \cite[\textsection 345]{doke1967textbook} -- not really VALENCE, ex. bind -- loosen
...
African Languages 1(3). 202–220.
    ...
    \item applicative
    (labeled \textit{ap}, 2K occurrences) -el- \cite[\textsection 310]{doke1967textbook} \cite[p. 109]{lombard1969handbook}
    ...
    \item rc reciprocal (-an 103 times)  \cite[\textsection 338]{doke1967textbook}
    ...
    \item VOICE: passive -iw- (labeled \textit{p}, 1K occurrences)
    ...
    The only tense suffix is the perfect affix -il-, which has a range of allomorphs depending on the preceding stem and valence/voice suffixes, if present \cite[\textsection 369]{doke1967textbook}, \cite[p. 167]{guma1971outline}, \cite[p. 116]{lombard1969handbook}.
    Common morphs in the Demuth corpus are -il- and -its-.
    
     
    \item MOOD: Mood (m\^{}, 37K times)
    
    In the Demuth corpus, the following mood endings are labeled:
