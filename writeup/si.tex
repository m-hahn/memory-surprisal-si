...
We represent language as a stochastic process of words $W = \dots w_{-2} w_{-1} w_0 w_{1} w_{2} \dots$, extending indefinitely both into the past and into the future \citep{doob1953stochastic,karlin2014first}.
...
%\paragraph{Assumption of Infinite Length}
...
%\mhahn{different way of stating this, if we go for the semi-infinite version}
%A simple way of stating this is as saying that, before the interaction starts, memory contains no information about what the speaker is going to say. Formally, this means that $m_0$ is statistically independent from $w_{1, 2, \dots}$:
%\begin{equation}\label{eq:no-mindreading}
%    m_0 \bot w_{1, 2, \dots}
%\end{equation}
...
As in the case of comprehension, we assume that $(w_t, m_t)_{t\in \mathbb{Z}}$ is stationary; however, we do \emph{not} assume the `No Mindreading' condition~(\ref{eq:no-mindreading}).
...
\end{equation}
which measures the information between $X_t$ and $X_0$ only to the degree that it is not redundant with the goal.
...
Conversely, one can construct processes where the unconditional mutual informations are 0 for all $t$, but where $\sum_t t I_t > 0$: Consider the process consisting of 2 random bits and their XOR. This one has nonzero $I_2$, but zero unconditional  mutual information $I[w_t, w_0]$ at all distances $t$.
