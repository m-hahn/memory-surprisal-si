...
%\footnote{TODO say something about Markov chain convergence: For each $t$, consider for each nonterminal $\tau$ the number $n_\tau$ of nodes with this nonterminal dominating $w_\tau$. This is a Markov chain.}
...
\captionof{figure}{Memory-surprisal tradeoffs computed with the PCFG estimator, comparing fitted grammars (blue) with baselines (red). For the random baselines, we provide the sample median and 95\% conficen intervals obtained with the binomial test.}\label{fig:resu-pcfg}
\end{center}
...
\begin{tabular}{lllllllll|lllllll}
Sbj. & Obj. & V & Val. & Voice & Tense & Mood \\ \hline
